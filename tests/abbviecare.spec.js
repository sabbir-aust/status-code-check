...
urls.forEach(({ country, url }) => {
  test(`has title and checks status code for Abbvie Care ${country} URL`, async ({ page }) => {
    ...
    try {
      ...
      await saveResponsesToExcel(responses, country, 'Care');
    } catch (error) {
      const statusCode = responses.find(response => response.url === mainUrl)?.status || 'Unknown';
      await logFailure(country, url, error.message, statusCode, 'Abbvie Care');
    }

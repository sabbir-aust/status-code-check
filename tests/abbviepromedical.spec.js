...
urls.forEach(({ country, url }) => {
  test(`has title and checks status code for AbbVie Pro Medical ${country} URL`, async ({ page }) => {
    ...
    await saveResponsesToExcel(responses, country, 'Medical');
    } catch (error) {
      const statusCode = responses.find(response => response.url === mainUrl)?.status || 'Unknown';
      await logFailure(country, url, error.message, statusCode, 'AbbVie Pro Medical');
    }
  });
});